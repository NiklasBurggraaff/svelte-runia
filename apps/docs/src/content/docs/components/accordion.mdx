...
<Example>
    <div slot="code">
        ```svelte "values={["item-1", "item-2"]}"
        <Accordion.Root type="multiple" values={["item-1", "item-2"]}>
            <Accordion.Item value="item-1">...</Accordion.Item>
            <Accordion.Item value="item-2">...</Accordion.Item>
        </Accordion.Root>
        ```
...
<Example>
    <div slot="code">
        ```svelte /Root (bind:value)/ "let value = $state("item-1");"
        // MyAccordion.svelte
        <script>
            import { Accordion } from "@niklasburggraaff/svelte-runia";
...
:::caution
If `collapsible = true` make sure to appropriately handle `values` being `null`, when all items are collapsed.
:::
